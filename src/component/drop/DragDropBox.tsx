...
export default function DragDropBox({ id, text, index, changePosition, className }:any) {
    const ref = useRef(null)
    // 因为没有定义收集函数，所以返回值数组第一项不要
    const [,drop] = useDrop({
        accept: 'DragDropBox', // 只对useDrag的type的值为DragDropBox时才做出反应
        hover: (item :any, monitor) => { // 这里用节流可能会导致拖动排序不灵敏
    ...
    return (
        ...
        <div 
        ref={ drag(drop(ref))  as LegacyRef<HTMLDivElement> | undefined } 
